...
/**
 * A loader for a private key resources.
 *
 * @author Carl Harris
 */
public class PrivateKeyLoader {
   ...
   *    read
   */
  public static PrivateKey loadKey(Path path,
    ...
    try {
      final PKCS8EncodedKeySpec spec =
          new PKCS8EncodedKeySpec(ByteArrayLoader.getContent(path, format));
  ...
  public static PrivateKey loadKeyResource(String resourceName,
      PublicKeyFormat format, PublicKeyType type,
    ...
    return PrivateKeyLoader.loadKey(Paths.get(url.toURI()),
        format, type);
  }
