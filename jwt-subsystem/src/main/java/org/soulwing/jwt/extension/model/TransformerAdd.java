...
/**
 * An add step handler for {@link TransformerDefinition}.
 *
 * @author Carl Harris
 */
class TransformerAdd extends AbstractAddStepHandler {
  ...
  private TransformerAdd() {
    super(new Parameters().addAttribute(TransformerDefinition.ATTRIBUTES));
  }
  ...
  protected void performRuntime(OperationContext context, ModelNode operation,
    ...
    final TransformerService service = TransformerService.builder()
    ...
    context.getCapabilityServiceTarget().addCapability(
        TransformerDefinition.TRANSFORMER_CAPABILITY
