...
/**
 * An add operation handler for the claim transform resource type.
 *
 * @author Carl Harris
 */
class ClaimTransformAdd extends AbstractAddStepHandler {
  ...
  private ClaimTransformAdd() {
    super(ClaimTransformDefinition.attributes());
  }
  ...
  protected void performRuntime(OperationContext context, ModelNode operation,
    ...
    final String name = ClaimTransformDefinition.NAME
        .resolveModelAttribute(context, model).asString();

    final ClaimTransformService service = new ClaimTransformService(name);

    final CapabilityServiceBuilder<ClaimTransformService> builder =
        context.getCapabilityServiceTarget()
            .addCapability(ClaimTransformDefinition.CLAIM_TRANSFORM_CAPABILITY, service);
