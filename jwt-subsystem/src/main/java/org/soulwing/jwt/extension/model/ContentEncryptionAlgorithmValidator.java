...
/**
 * A {@link ModelTypeValidator} for the {@link JWE.ContentEncryptionAlgorithm}
 * type.
 *
 * @author Carl Harris
 */
class ContentEncryptionAlgorithmValidator extends ModelTypeValidator
    implements AllowedValuesValidator {

  private final Set<JWE.ContentEncryptionAlgorithm> allowedValues;
  private final Set<String> allowedTokens;

  public static final ContentEncryptionAlgorithmValidator INSTANCE =
      new ContentEncryptionAlgorithmValidator();

  private ContentEncryptionAlgorithmValidator() {
    super(ModelType.STRING);
    this.allowedValues = Arrays.stream(JWE.ContentEncryptionAlgorithm.values())
        .collect(Collectors.toCollection(LinkedHashSet::new));
    this.allowedTokens = allowedValues.stream()
        .map(JWE.ContentEncryptionAlgorithm::toToken)
        .collect(Collectors.toCollection(LinkedHashSet::new));
  }
  ...
  public void validateParameter(String parameterName, ModelNode value)
    ...
    if (type == ModelType.STRING) {
      ...
      try {
        algorithm = JWE.ContentEncryptionAlgorithm.of(token);
      }
      catch (IllegalArgumentException iex1) {
        try {
          algorithm =
              JWE.ContentEncryptionAlgorithm.of(token.toUpperCase(Locale.ENGLISH));
        }
