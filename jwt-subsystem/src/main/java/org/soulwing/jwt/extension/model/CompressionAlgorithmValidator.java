...
/**
 * A {@link ModelTypeValidator} for the {@link JWE.CompressionAlgorithm} type.
 *
 * @author Carl Harris
 */
class CompressionAlgorithmValidator extends ModelTypeValidator
  ...
  public static final CompressionAlgorithmValidator INSTANCE =
      new CompressionAlgorithmValidator();

  private CompressionAlgorithmValidator() {
    super(ModelType.STRING);
    this.allowedValues = Arrays.stream(JWE.CompressionAlgorithm.values())
        .collect(Collectors.toCollection(LinkedHashSet::new));
    this.allowedTokens = allowedValues.stream()
        .map(JWE.CompressionAlgorithm::toToken)
        .collect(Collectors.toCollection(LinkedHashSet::new));
  }
  ...
  public void validateParameter(String parameterName, ModelNode value)
    ...
    if (type == ModelType.STRING) {
      ...
      try {
        algorithm = JWE.CompressionAlgorithm.of(token);
      }
      catch (IllegalArgumentException iex1) {
        try {
          algorithm =
              JWE.CompressionAlgorithm.of(token.toUpperCase(Locale.ENGLISH));
        }
