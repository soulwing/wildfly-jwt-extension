...
/**
 * Unit tests for {@link SecretKeyAdd}.
 *
 * @author Carl Harris
 */
public class SecretKeyAddTest {
  ...
  public void setUp() throws Exception {
    operation.set(ModelDescriptionConstants.OP_ADDR,
        PathAddress.pathAddress(Constants.SECRET_KEY, "secretKey").toModelNode());

    model.get(Constants.ID).set("id");
    model.get(Constants.TYPE).set("type");
    model.get(Constants.LENGTH).set(128);
  ...
  public void testPerformBoottime() throws Exception {
    context.checking(new Expectations() {
      {
        ...
        oneOf(target).addCapability(with(any(RuntimeCapability.class)),
            with(any(SecretKeyService.class)));
    ...
    SecretKeyAdd.INSTANCE.performRuntime(operationContext, operation, model);
  }
